...
PAPER EDGE: where the photographic anchor meets the cream paper, render a visible hand-torn fibrous edge: irregular hand-ripped contour with shallow notches, uneven rises, soft scallops and occasional longer fiber pulls; a narrow feathered fringe of exposed paper fibers; slight local abrasion and dry pigment loss; asymmetric tearing with one or two stronger pressure points; the fibrous band about 1%-4% of the short edge, visible along roughly 35%-70% of the photo perimeter; flat scan behavior, no lifted-paper depth, no uniform frame, no sticker border, no drop shadows.
...
EXPRESSION: inspect the photograph and build an internal distillation card: semantic nucleus (the smallest subject or relationship that gives it meaning), core subject, dominant gesture, one spatial cue, material and weather, and emotional residue. Formulate one expressive proposition, choose ONE central tension that already exists or credibly emerges from the source (intimacy/distance, shelter/confinement, movement/stillness, smallness/vastness, warmth/coldness, memory/disappearance, order/growth, visibility/concealment, permanence/fragility), transform one source-derived object, spatial relationship or material behavior into ONE central visual metaphor, embody it through scale, direction, edge, color and material, and leave one relationship deliberately unanswered as an interpretive opening. Preserve only 2-4 source anchors (a shape, a gesture, a color relationship, a spatial cue); do not preserve the original composition by default. Subject evidence: {feature_block}. Remove any element whose only function is to look artistic.

CANVAS: 3:5 portrait or 5:3 landscape following the source orientation; warm paper with paper fibers, dry ink, grain and flat scan texture; generous quiet space; no mockup, no frame, no photographic material.
